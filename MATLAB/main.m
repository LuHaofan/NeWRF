...
numAPs = 1;
numSTAs = length(STAs);
%% Define Object Area and Remove collided STAs
obj_dims = [-0.7,0.7;-0.7,0.7;0.0,1.3];
collided_sta_id = [];
for i = 1:size(STAs, 2)
    if isCollide(obj_dims, STAs(i).AntennaPosition)
        collided_sta_id = [collided_sta_id, i];
    end
end
STAs(collided_sta_id) = [];
